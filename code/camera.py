...
class Camera(object):
    ...
    def __init__(self, env, near=0.1, far=100.0, image_size=448, dist=5.0,
            phi=np.pi/5, theta=np.pi, fov=35, random_position=False, fixed_position=False, restrict_dir=False, real_data=False):

        camera_mount_actor = env.scene.create_actor_builder().build_kinematic()
        self.env = env

        # set camera intrinsics
        self.camera = env.scene.add_mounted_camera('camera', camera_mount_actor,
                                                   Pose(), 1920, 1080, 0,
                                                   np.deg2rad(fov), near, far)
    ...
    def get_observation(self):
        ...
        mask = (position[:, :, 2] < -0.1)
        position[:, :, -1] = mask
        points_opengl = position[..., :3][mask]
        model_matrix = self.camera.get_model_matrix()
        points_world = points_opengl @ model_matrix[:3, :3].T + model_matrix[:3, 3]

        return rgb, points_world, position

    def get_depth(self):
        depth = self.camera.get_depth().astype(np.float32)
        return depth

    def position_to_world(self, position):
        model_matrix = self.camera.get_model_matrix()
        points_world = position @ model_matrix[:3, :3].T + model_matrix[:3, 3]
        return points_world

    def compute_camera_XYZA(self, position):
    ...
    def get_handle_mask(self):
        ...
        part_seg = self.camera.get_uint32_texture('Segmentation')
        part_seg = part_seg[..., 1].astype(np.uint8)
