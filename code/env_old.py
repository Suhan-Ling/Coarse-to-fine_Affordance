...
class Env(object):
    ...
    def load_object(self, urdf, material, state='closed', target_part_id=-1, density=0):
        ...
        for j in self.object.get_joints():
            if j.get_dof() == 1:
                ...
                if state == 'closed':
                    ...
                    min = l + (r - l) / 6
                    max = l + (r - l) / 3
                    joint_angles.append(float(get_random_number(min, max)))
    ...
    def get_material(self, static_friction, dynamic_friction, restitution):
        return self.engine.create_physical_material(4, 4, restitution)
