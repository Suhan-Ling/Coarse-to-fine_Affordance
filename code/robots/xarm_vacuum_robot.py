...
class XarmVacuumRobot(object):
    def __init__(self, env, urdf, material, use_lock_motion=True):
        self.env = env
        ...
        self.arm_joints = [joint for joint in self.robot.get_joints() if
                joint.get_dof() > 0 and not joint.get_name().startswith("xarm_vacuum_gripper_link")]
    ...
    def start_sunction(self, suction_drive):
        if self.use_lock_motion:
    ...
    def release_suction(self, suction_drive):
        # Release suction
        if self.use_lock_motion:
    ...
    def compute_joint_velocity_from_twist(self, twist: np.ndarray) -> np.ndarray:
        ...
        ee_jacobian = np.zeros([6, self.robot.dof])
        ee_jacobian[:3, :] = dense_jacobian[self.end_effector_index * 6 - 3: self.end_effector_index * 6, :self.robot.dof]
        ee_jacobian[3:6, :] = dense_jacobian[(self.end_effector_index - 1) * 6: self.end_effector_index * 6 - 3, :self.robot.dof]
        ...
        try:
            ...
            try:
                ...
                print(ee_jacobian)
                inverse_jacobian = np.random.rand(6, 6)
    ...
    def internal_controller(self, qvel: np.ndarray) -> None:
        ...
        target_qpos = qvel * self.timestep + self.robot.get_drive_target()[:]
    ...
    def move_to_target_pose(self, target_ee_pose: np.ndarray, num_steps: int, visu=None, vis_gif=False, vis_gif_interval=200, cam=None):
        """
    ...
    def wait_n_steps(self, n: int, visu=None, vis_gif=False, vis_gif_interval=1, cam=None):
        if visu:
            waypoints = []
