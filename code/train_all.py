...
def train(conf, train_data_list, val_data_list):
    ...
    data_features = ['gt_labels', 'up', 'forward', 'pc_near', 'pc_far', 'far_valid']

    # load network model
    model_def = utils.get_model_module(conf.model_version)

    # create models
    network = model_def.Critic(feat_dim=conf.feat_dim)
    if conf.load:
        data_to_restore = torch.load(os.path.join("../logs", conf.load_dir, 'ckpts', f"{conf.load_epoch}-network.pth"))
    ...
    val_dataset.load_data('../data/0515_pull_StorageFurniture_1', real=conf.real)
    train_dataset.load_data('../data/0515_pull_StorageFurniture_2', real=conf.real)
    train_dataset.load_data('../data/0515_pull_StorageFurniture_3', real=conf.real)
    train_dataset.load_data('../data/0515_pull_StorageFurniture_4', real=conf.real)
    train_dataset.load_data('../data/0515_pull_StorageFurniture_5', real=conf.real)
    train_dataset.load_data('../data/0515_pull_StorageFurniture_6', real=conf.real)
    train_dataset.load_data('../data/0515_pull_StorageFurniture_7', real=conf.real)
    train_dataset.load_data('../data/0515_pull_StorageFurniture_8', real=conf.real)
    ...
    utils.printout(conf.flog,
                   'train_num_batch: %d, val_num_batch: %d' % (train_num_batch, val_num_batch))
    ...
    for epoch in range(start_epoch, conf.epochs):
        ...
        for train_batch_idx, batch in train_batches:
            train_fraction_done = (train_batch_idx + 1) / train_num_batch
            train_step = epoch * train_num_batch + train_batch_idx
            ...
            # save checkpoint
            if epoch % 5 == 0 and train_batch_idx == 0:
                with torch.no_grad():
            ...
            losses = critic_forward(batch=batch, data_features=data_features, network=network,
                                    conf=conf, is_val=False, step=train_step, epoch=epoch,
                                    batch_idx=train_batch_idx, num_batch=train_num_batch,
                                    start_time=start_time, log_console=log_console,
                                    log_tb=not conf.no_tb_log, tb_writer=train_writer,
            ...
            while val_fraction_done <= train_fraction_done and val_batch_idx + 1 < val_num_batch:
                val_cnt += 1
                val_batch_idx, val_batch = next(val_batches)

                val_fraction_done = (val_batch_idx + 1) / val_num_batch
                ...
                with torch.no_grad():
                    # forward pass (including logging)
                    total_loss = critic_forward(batch=val_batch, data_features=data_features,
                                                network=network, conf=conf, is_val=True,
                                                step=val_step, epoch=epoch, batch_idx=val_batch_idx,
...
def critic_forward(batch, data_features, network, conf,
                   is_val=False, step=None, epoch=None, batch_idx=0, num_batch=1,
                   start_time=0.0, log_console=False, log_tb=False, tb_writer=None, lr=None):
    ...
    valid = batch[data_features.index('far_valid')]

    far_valid = []
    for ww in range(len(valid)):
        if valid[ww]:
            far_valid.append(ww)
    if len(far_valid) == 0:
        far_valid = None
    ...
    loss = network.get_ce_loss(dir, f_dir, pc_near, pc_far, gt_labels,
                               near_only=False, far_valid=far_valid, lbd=conf.lbd)  # B x 2, B x F x N
    ...
    with torch.no_grad():
        ...
        if log_console:
            utils.printout(conf.flog,
                           ...
                           f'''{data_split:^10s} '''
                           f'''{batch_idx:>5.0f}/{num_batch:<5.0f} '''
                           f'''{100. * (1 + batch_idx + num_batch * epoch) / (num_batch * conf.epochs):>9.1f}%      '''
                           f'''{lr:>5.2E} '''
                           f'''{loss.item():>10.5f}'''
                           )
            conf.flog.flush()
